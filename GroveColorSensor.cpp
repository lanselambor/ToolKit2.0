...
{  
  i2cWriteReg(sensorAddress_, REG_TIMING, triggerMode_);
  i2cWriteReg(sensorAddress_, REG_INT_SOURCE, interruptSource_);
  i2cWriteReg(sensorAddress_, REG_INT, interruptMode_);
  i2cWriteReg(sensorAddress_, REG_GAIN, gainAndPrescaler_);
  i2cWriteReg(sensorAddress_, REG_CTL, CTL_DAT_INIITIATE);  
}

void GroveColorSensor::i2cWriteReg(uint8_t addr, uint8_t reg, uint8_t value)
{
